...
firstHalf str | length str `mod` 2 == 0 = take (findHalf str) str
              | otherwise = take (findHalf str + 1) str
           where findHalf str = length str `div` 2
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x <= max) karr 
        where karr = map (\x -> k ^ x) [0..max]
...
while check update value | check value == True = while check update (update value)
                         | otherwise = value
...
whileRight check x = case check x of 
     Left val -> val
     Right var -> whileRight check var
...
joinToLength n str = [ str1 ++ str2 | str1 <- str, str2 <- str, length (str1 ++ str2) == n]  
...
multiCompose [] x = x 
multiCompose f x = multiCompose (init f) (last(f) x)
...
multiApp f g x = f (map (\op -> op x) g)
...
interpreter string = interpreter' (0,0) string []

interpreter' :: (Int, Int) -> [String] -> [String] -> [String]
interpreter' (x,b) [] res = reverse res
interpreter' (x,b) (y:ys) res = 
    case y of "up" -> interpreter' (x , b + 1) ys res
              "down" -> interpreter' (x, b - 1) ys res
              "left" -> interpreter' (x - 1, b) ys res
              "right" -> interpreter' (x + 1, b) ys res
              "printX" -> interpreter' (x,b) ys (show x : res)
              "printY" -> interpreter' (x,b) ys (show b : res)
