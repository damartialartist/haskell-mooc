...
palindromify s = palin' s False

palin' :: String -> Bool -> String
palin' s True = s
palin' s b | s == reverse s = palin' s True
           | otherwise = palin'(reverse (drop 1 (reverse (tail s)))) (False)
